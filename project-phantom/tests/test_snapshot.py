...
def test_snapshot_dedups_unchanged_content(tmp_path):
    ...
    engine = SnapshotEngine(store_dir)

    first = engine.take_snapshot(data_dir)
    objects_after_first = list(engine.objects_dir.iterdir())
    ...
    assert list(engine.objects_dir.iterdir()) == objects_after_first
    ...
    assert len(list(engine.objects_dir.iterdir())) == 2
...
def test_restore_reconstructs_files(tmp_path):
    ...
    engine = SnapshotEngine(tmp_path / "store")
